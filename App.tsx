...
const App: React.FC = () => {
      ...
      default:
        if (appState === AppState.LOADING) {
          return (
            <div className="py-12">
              <Loader />
              <p className="text-center text-gray-500 mt-4">Analyzing: {auditingUrl} • Elapsed time: {elapsedSeconds}s</p>
              {!backendAvailable && <p className="text-center text-orange-500 text-sm mt-2">⚠️ Backend unavailable - showing demo results</p>}
            </div>
          );
        }
          ...
          return (
            <div className="text-center py-32 animate-fade-in">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <button onClick={handleReset} className="px-6 py-3 bg-black text-white rounded-xl font-medium hover:bg-gray-800 transition-colors">Try Again</button>
                <button onClick={() => setCurrentPage(Page.LOGIN)} className="px-6 py-3 border border-gray-200 text-gray-700 rounded-xl font-medium hover:border-gray-400 transition-colors">Test Login Page</button>
              </div>
            </div>
          );
        }
  ...
  return <Layout currentPage={currentPage} setPage={setCurrentPage}>{renderContent()}</Layout>;
};
